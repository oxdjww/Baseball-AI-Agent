...
import com.kbank.kbaseball.game.analysis.GameAnalysisPromptBuilder;
import com.kbank.kbaseball.game.message.GameMessageFormatter;
import com.kbank.kbaseball.external.naver.NaverRosterClient;
import com.kbank.kbaseball.external.naver.NaverGameRecordClient;
...
public class GameAnalysisTasklet {
    ...
    public void execute(ScheduledGameDto schedule, RealtimeGameInfoDto info) {
        if (!featureToggleService.isEnabled(FeatureToggleService.AI_ANALYSIS)) {
            log.info("[GameAnalysis][execute] AI_ANALYSIS feature is disabled. Skipping.");
            return;
        }
